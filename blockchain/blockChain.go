...
func NewBlockChain() *BlockChain {
	...
	db.Update(func(tx *bolt.Tx) error{
		...
		if len(lastHash) == 0 {//桶当中没有lasthash记录,需要新建创世区块并保存
			//创世区块
		...
		}else {//桶当中已有lasthash的记录,不在需要创世区块,只需要读取即可
...
//该方法用于遍历区块链chain.db,并将所有区块查出,并返回
func (bc BlockChain) QueryAllBlocks() ([]*Block,error) {
	blocks := make([]*Block,0)//是一个切片容器,用于盛放查询到的区块
	db := bc.BoltDb
	var err error
	//从chain.db文件查询所有的区块
...
//该方法用于根据区块高度查询区块信息
func (bc BlockChain) QueruBlockHeight(height int64) (*Block,error){
	if height<0 {
		return nil,nil
	}
	db := bc.BoltDb
...
func (bc *BlockChain) SaveData(data []byte) (Block,error){
    ...
    db.Update(func(tx *bolt.Tx) error{
    	bucket := tx.Bucket([]byte(BUCKET_NAME))
		//把新创建的区块存入到blotdb数据库当中
    	bucket.Put(newBlock.Hash,newBlock.Serialize())
    	//更新lasthash对应的值,更新为最新存储的区块的hash值
    	bucket.Put([]byte(LAST_HASH),newBlock.Hash)
    	bc.LastHash = newBlock.Hash//将区块链实例的LASTHASH的值更新为最新的区块的HASH

    	return nil
	})
	//返回值语句,newBlock'err,其中可嗯呢该包涵错误信息
	return newBlock,err
